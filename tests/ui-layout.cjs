...
(async () => {
    ...
    try {
        ...
        for (const [width, height] of [[837, 805], [390, 844], [390, 320]]) {
            ...
            await page.locator('#resetEventDraft').click();
            assert.equal(await page.locator('#eventForm [name="title"]').inputValue(), '');
            await page.locator('#eventForm [name="title"]').fill('操作栏回归日程');
        ...
        console.log('PASS actual reset/save/cancel, meeting steps/drafts, library/profile saves, toast click-through and short sidebar navigation');
