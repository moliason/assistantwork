...
弹窗布局测试覆盖全部 8 类弹窗、会议各步骤、长列表及 9 种窗口尺寸，分别检查正文滚动前后按钮的完整边界与点击命中；另外实际执行保存、取消和向导操作，并检查提示消息不会拦截导航点击。`TEST_URL` 可指定已部署网址用于线上复测。

数据安全：已移除退出及表单重置按钮，重新登录全新账号不再清空浏览器中已存数据。`node tests/data-safety.cjs` 检查已有数据在两种登录入口下保持不变，以及表单填写草稿在关闭、刷新后保留。数据仍仅存于当前浏览器；清除网站数据仍会丢失，已创建会议的持久化问题不在此次修改范围内。
